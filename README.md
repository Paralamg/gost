...
Вы описываете документ как последовательность элементов — заголовки, абзацы, рисунки,
таблицы, формулы. Всё остальное библиотека берёт на себя:

- **Оформление по ГОСТ 7.32-2017** — шрифт, интервалы, красная строка, поля и подписи
  проставляются автоматически, без ручной настройки каждого абзаца.
- **Сквозная нумерация** — номера рисунков, таблиц и формул считаются сами и не сбиваются
  при вставке нового элемента в середину документа.
- **Формулы из LaTeX** — вставляются настоящим объектом «Уравнение», а не картинкой:
  их можно править прямо в Word и печатать без потери качества.
- **Перенос длинных таблиц** — с повтором шапки и надписью «Продолжение таблицы N».

Это надстройка над `python-docx`: он умеет абзацы и таблицы, но ничего не знает про ГОСТ.
