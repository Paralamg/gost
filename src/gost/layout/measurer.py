...
class WordMirror:
    ...
    def append(self, chunk: Path) -> None:
        ...
        внутри зеркала — в документе такого абзаца нет. Абзацы копятся по одному
        на таблицу, всё, что ниже, съезжает, и таблица меряется не там, где
        окажется у читателя.

        Поэтому вставляем перед ними: кусок встаёт выше, а они вытесняются вниз,
        под последнюю таблицу, где меряться уже нечему. Удалять их нельзя — без
        абзаца вставке не за что зацепиться, и Word сливает первый абзац куска с
        последним абзацем зеркала.

        Хвост ищется заново на каждой вставке, и это стоит прохода по зеркалу:
        абзацы Word отдаёт только перебором от начала документа. Запоминать
        позицию хвоста самим не вышло — Word приписывает свой абзац внутрь
        вставленного диапазона, и вычисленный якорь разъезжается с документом.
        """
    ...
    def __first_trailing_empty_paragraph(self):
        ...
        Идём от конца через Last и Previous: Paragraphs.Count и обращение к
        коллекции по индексу Word считает перебором с начала документа, и это
